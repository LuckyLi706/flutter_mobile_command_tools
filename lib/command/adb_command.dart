...
class AdbCommand extends BaseCommand {
  ...
  Future<CommandResultModel<T>?> runCommand<T>(String command,
    ...
    return super.runCommand<T>(command,
  ...
  List<String> checkArguments(List<String> arguments) {
    if (Constants.currentDevice.isNotEmpty) {
      if (arguments[0] != AdbCommandType.ADB_CONNECT_DEVICES.value &&
          arguments[0] != Constants.ADB_WIRELESS_DISCONNECT &&
          arguments[0] != Constants.ADB_WIRELESS_CONNECT &&
          arguments[0] != AdbCommandType.ADB_VERSION.value) {
  ...
  CommandResultModel<T> parseData<T>(
    ...
    /// 解析连接设备的命令
    if (command == AdbCommandType.ADB_CONNECT_DEVICES.value) {
      if (data.contains("List of devices attached")) {
        ...
        if (currentDevices.length > 0) {
          return CommandResultModel(true, currentDevices as T);
        } else {
          return CommandResultModel.error(false, null, "无设备连接");
        }
      } else {
      ...
      /// 解析获取包名的命令
    } else if (command == AdbCommandType.ADB_GET_PACKAGE.value) {
    ...
    //     return CommandResultModel(true, value);
    // }
    return CommandResultModel(true, data as T);
  }
}
